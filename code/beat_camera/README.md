...
Reference implementation of **BEAT** (*Beat-wise Encoding for Autoregressive
Transformers*) — a single LLaMA backbone over a unified 593-token vocabulary that
generates both **solo piano** and **multi-track** symbolic music.

This repository contains the model, tokenizers, data-preparation pipeline, and
inference scripts. Released under the Apache 2.0 license.
...
```bash
python -m beat_camera.scripts.generate_piano \
    --checkpoint checkpoints/piano/backbone.pt \
...
folder of MIDI (or MusicXML) into the NPZ format the model trains on. See
[`data_prep/README.md`](data_prep/README.md) for formats and the source each
...
python -m beat_camera.data_prep.xml2multitracknpz <xml_dir> --output_dir <out_dir>
```
...
accelerate launch --multi_gpu --num_processes 4 \
    -m beat_camera.scripts.train_multitrack
```
