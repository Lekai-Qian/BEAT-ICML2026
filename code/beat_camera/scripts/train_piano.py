...
Usage:
  # single GPU
  CUDA_VISIBLE_DEVICES=0 python -m beat_camera.scripts.train_piano

  # multi-GPU (e.g. 4×)
  CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --multi_gpu --num_processes=4 \\
      -m beat_camera.scripts.train_piano

  # resume from existing checkpoint
  python -m beat_camera.scripts.train_piano --resume checkpoints/piano/backbone_best.pt

Tensorboard logs live under `<log_dir>/<run_name>/` — open with:
  tensorboard --logdir beat_camera/logs/piano --port 6006
"""
